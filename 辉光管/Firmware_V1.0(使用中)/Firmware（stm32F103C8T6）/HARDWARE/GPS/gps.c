...
{
	if(gps_data->utc.year != 0 && gps_data->utc.date != 0)
		return TRUE;
	else
		return FALSE;
}
...
{
	int SetTime[7];
	if(Correct_time(gps_data) == TRUE)
	{
		SetTime[6] = gps_data->utc.year;
		SetTime[5] = gps_data->utc.month;
		SetTime[4] = gps_data->utc.date;
		SetTime[2] = gps_data->utc.hour;
		SetTime[1] = gps_data->utc.min;
		SetTime[0] = gps_data->utc.sec;
...
void gps_io_cfg(void)
{
	...
	GPIO_InitTypeState.GPIO_Pin = GPIO_Pin_1;
	...
	GPIO_InitTypeState.GPIO_Pin = GPIO_Pin_4;
	GPIO_Init(GPIOA, &GPIO_InitTypeState);
}
/********************************************************************************************
GPS数据解析
...
void GPRMC_Analysis(gps_process_data *gps_data,unsigned char *buf)
{
	...
	{
		temp=Data_Extraction(address_buf+end_buf,&decimal_places);		 	 
		gps_data->utc.date=temp/10000;
		gps_data->utc.month=temp%10000/100;
		gps_data->utc.year=temp%100;
	}		 
}  
