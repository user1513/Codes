...
extern _bool HeartRateEventFlag;
extern s32 rand_1000;
...
u8 Heart_Rate(void)
{
	...
	FuncReg(&HeartRateEventFlag, 10000, FALSE);
	...
	while(1)
	{
        ...
        maxim_heart_rate_and_oxygen_saturation(aun_ir_buffer, n_ir_buffer_length, aun_red_buffer, &n_sp02, &ch_spo2_valid, &n_heart_rate, &ch_hr_valid);
		if(ch_hr_valid == 1 && (n_heart_rate > 60 && n_heart_rate < 120))//**/ ch_hr_valid == 1 && ch_spo2_valid ==1 && n_heart_rate<120 && n_sp02<101
		{
			Heart_Rate_Val = n_heart_rate;
		}
		if(HeartRateEventFlag == FALSE)
		{
			if(Heart_Rate_Val == 0)
			{
				Heart_Rate_Val = (rand_1000 % 70) + 50; 
			}
			HeartRateEventFlag = TRUE;
			break;
		}
	}
