...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagTime)
    {
        ...
        info->num++;
        dataPoints->valueTime = gizStandardDecompressionValue(Time_BYTEOFFSET,Time_BITOFFSET,Time_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valueTime >= Time_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueTime Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Time_BYTEOFFSET,Time_BITOFFSET,Time_LEN,(uint8_t *)devStatusPtr,dataPoints->valueTime);  
    }
    ...
    devStatusPtr->valueGas = exchangeBytes(gizY2X(Gas_RATIO,  Gas_ADDITION, dataPoints->valueGas)); 
    devStatusPtr->valueHeart_Rate = exchangeBytes(gizY2XFloat(Heart_Rate_RATIO,  Heart_Rate_ADDITION, dataPoints->valueHeart_Rate));     
