...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      ...
      case EVENT_Time:
        currentDataPoint.valueTime = dataPointPtr->valueTime;
        GIZWITS_LOG("Evt: EVENT_Time %d\n", currentDataPoint.valueTime);
...
void userHandle(void)
{
 /*
    ...
    currentDataPoint.valuerssi = ;//Add Sensor Data Collection
