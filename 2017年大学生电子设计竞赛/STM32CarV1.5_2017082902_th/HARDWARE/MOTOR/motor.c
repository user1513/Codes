...
void Motor_IO_Init(void)
{
	...
	GPIOB->CRH |= 0X33330000;	//PB.15 PB.14 PB.13 PB.12 推挽输出
	GPIOB->ODR |= 0<<12;		//PB.12 输出低
	GPIOB->ODR |= 0<<13;		//PB.13 输出低
	GPIOB->ODR |= 0<<14;		//PB.14 输出低
	GPIOB->ODR |= 0<<15;		//PB.15 输出低
}
...
//	TIM4->CR1 |= 0x01;   		//使能定时器 		
	...
	RCC->APB1ENR |= 1<<2;       //TIM4时钟使能    
	RCC->APB2ENR |= 1<<3;       //PORTB时钟使能   
	GPIOB->CRL &= 0X00FFFFFF;   //PORTB6 7推挽输出
	GPIOB->CRL |= 0XBB000000;   //PORTB6 7推挽输出
	TIM4->ARR = arr;			//设定计数器自动重装值 
	TIM4->PSC = psc;			//预分频器不分频
	TIM4->CCMR1 |= 6<<4;		//CH1 PWM1模式	
	TIM4->CCMR1 |= 6<<12; 		//CH2 PWM1模式	
