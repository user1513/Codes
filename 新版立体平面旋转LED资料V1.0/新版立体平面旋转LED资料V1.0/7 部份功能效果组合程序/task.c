...
Ò£¿ØÆ÷°´¼ü3 £ºÆ½ÃæÊý×ÖÊ±ÖÓ+²àÃæÏÔÊ¾ÎÄ×Ö
Ò£¿ØÆ÷°´¼ü4 £ºÆ½ÃæÍ¼Æ¬+²àÃæÏÔÊ¾ÎÄ×Ö
*************************************************************/
#include "NEW_8051.H"
#include "intrins.h"
#include "task.h"
...
void int0() interrupt 0	//Ê¹ÓÃÍâ²¿ÖÐ¶Ï0
{
	...
	else if(ucMode==4)
	{
		...
		{
			Row=0;
			OffsetAdder=OffsetAdder+720;//Ò»¸öÍ¼Æ¬720¸ö×Ö½Ú
	        if(OffsetAdder>2880) //ÓÐ¼¸ÕÅÍ¼Æ¬ ¾ÍÐèÒª720 * (¼¸ÕÅ-1)
		     OffsetAdder=0;
		}
		Offset++;
		if(Offset>480)//²àÃæÓÐ¶àÉÙÎÄ×Ö Õâ¸öÖµ¾Í¶ÔÓ¦¸Ä³É  ¶àÉÙ×Ö*16 
...
void display_pic()
{
	...
	if(DISP_LINE<180)
	{
		if(Time0_flag)
		{
			...
			else if(ucMode==4) //Æ½ÃæÍ¼Æ¬+²àÃæÎÄ×Ö
			{
			 	Close();		
		...
		if(BIT_timeout==1)	//ÆðµãÅÐ¶Ï
	 	{			
			...
			if( new_code ) //ÓÐºìÍâ°´¼ü
			{
				...
				{	
				   case 0x0C:
					ucMode=0x01;//Æ½ÃæÏÔÊ¾ÎÄ×ÖÄ£Ê½+²àÃæÊ±ÖÓ
				//	TR0 = 0;	
				break;
				...
				break;
				case 0x5E:
					ucMode=0x03;//Æ½ÃæÊ±ÖÓÄ£Ê½+²àÃæÎÄ×Ö
				//	TR0=0;			 	
				break;
				case 0x08:
					ucMode=0x04;//Æ½ÃæÏÔÊ¾Í¼Æ¬+²àÃæÎÄ×Ö
					//TR0 = 1;
				break;
				/**********************************************/
					case 0x40:	 //µ÷Ê±¼Ó
			...
			if(ucMode==1)
			{
				...
				Show_line_time();//²àÃæÏÔÊ¾Ê±¼ä+Æ½ÃæÎÄ×Ö
			}
    		else if(ucMode==3)
			{
				du1302P();//¶ÁÈ¡Ê±ÖÓÐ¾Æ¬ µÄÊ±ÖÓÊý¾Ý
				Show_plane_time();//Æ½ÃæÏÔÊ¾Ê±¼ä+²àÃæÎÄ×Ö
			}
