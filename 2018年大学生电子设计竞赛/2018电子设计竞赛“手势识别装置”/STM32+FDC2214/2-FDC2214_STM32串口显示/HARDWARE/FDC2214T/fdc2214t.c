...
{					     
	...
	GPIO_ResetBits(GPIOB,GPIO_Pin_13|GPIO_Pin_14); 	//PB6,PB7 输出高
}
...
void FDCt_IIC_Start(void)
{
	...
	FDCt_IIC_Delay();
	FDCt_IIC_SCL=0;//钳住I2C总线，准备发送或接收数据 
}	  
//产生IIC停止信号
...
{ 
    FDCt_IIC_Start(); 
	FDCt_IIC_Send_Byte((FDC2214_ADDR<<1)|0);//发送器件地址+写命令	
	if(FDCt_IIC_Wait_Ack())	//等待应答
	{
		FDCt_IIC_Stop();		 
		return 1;		
	}
    FDCt_IIC_Send_Byte(reg);//写寄存器地址
    FDCt_IIC_Wait_Ack();		 //等待应答 
	FDCt_IIC_Send_Byte(MSB);  //发送数据1
	if(FDCt_IIC_Wait_Ack())	 //等待ACK
	{
		FDCt_IIC_Stop();	 
		return 1;		 
	}		 
	FDCt_IIC_Send_Byte(LSB);  //发送数据2
	if(FDCt_IIC_Wait_Ack())	 //等待ACK
	{
		FDCt_IIC_Stop();	 
		return 1;		 
...
u16 FDCt_Read(u8 reg)
{
   ...
   FDCt_IIC_Start(); 
    ...
    FDCt_IIC_Wait_Ack();		//等待应答
    FDCt_IIC_Start();
	FDCt_IIC_Send_Byte((FDC2214_ADDR<<1)|1);//发送器件地址+读命令	
    FDCt_IIC_Wait_Ack();		//等待应答 
	res=FDCt_IIC_Read_Byte(1)<<8;//读取数据,发送ACK
	  
	res|=FDCt_IIC_Read_Byte(0);//读取数据,发送nACK
    FDCt_IIC_Stop();			//产生一个停止条件
	return res;		
}
...
u8 FDC2214t_Init(void)
{
	...
	{
		//设置Set_FDC2214寄存器
		Sett_FDC2214(RCOUNT_CH0,0x34,0xFB);//参考计数转换间隔时间（T=(RCOUNT_CH0*16)/Frefx）
		Sett_FDC2214(RCOUNT_CH1,0x34,0xFB);
		Sett_FDC2214(RCOUNT_CH2,0x34,0xFB);
		
		Sett_FDC2214(SETTLECOUNT_CH0,0x00,0x1B);//转换之前的稳定时间（T=(SETTLECOUNT_CHx*16)/Frefx）
		Sett_FDC2214(SETTLECOUNT_CH1,0x00,0x1B);
		Sett_FDC2214(SETTLECOUNT_CH2,0x00,0x1B);
		
		Sett_FDC2214(CLOCK_DIVIDERS_C_CH0,0x20,0x02);//选择在0.01MHz ~ 10MHz的传感器频率
		Sett_FDC2214(CLOCK_DIVIDERS_C_CH1,0x20,0x02);//Frefx = Fclk = 43.4MHz/2(2分频)
		Sett_FDC2214(CLOCK_DIVIDERS_C_CH2,0x20,0x02);//CHx_REF_DIVIDER=2;CHx_FIN_SEL=2
		
		Sett_FDC2214(DRIVE_CURRENT_CH0,0x78,0x00);//0.146ma（传感器时钟建立+转换时间的驱动电流）
		Sett_FDC2214(DRIVE_CURRENT_CH1,0x78,0x00);
		Sett_FDC2214(DRIVE_CURRENT_CH2,0x78,0x00);
		
		Sett_FDC2214(ERROR_CONFIG,0x00,0x00);//全部禁止错误汇报
		
		Sett_FDC2214(MUX_CONFIG,0xC2,0x0D);//通道0，1，2 ，3；选择10Mhz为超过振荡槽振荡频率的最低设置，多通道，四通道
		
		Sett_FDC2214(CONFIG,0x14,0x01);//激活模式，使用内部振荡器做参考频率，INTB引脚会随状态寄存器更新被置位
	}
	else return 1;
