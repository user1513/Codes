...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuesoilhum != cur->valuesoilhum)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesoilhum Changed\n");
            ret = 1;
        }
    }
    if(last->valueEnvironmentalScienceHum != cur->valueEnvironmentalScienceHum)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueEnvironmentalScienceHum Changed\n");
            ret = 1;
        }
    }
    if(last->valueEnvironmentalScienceTemp != cur->valueEnvironmentalScienceTemp)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueEnvironmentalScienceTemp Changed\n");
            ret = 1;
        }
    }
    if(last->valuesignal != cur->valuesignal)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesignal Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuesoilhum = gizY2X(soilhum_RATIO,  soilhum_ADDITION, dataPoints->valuesoilhum); 
    devStatusPtr->valueEnvironmentalScienceHum = gizY2X(EnvironmentalScienceHum_RATIO,  EnvironmentalScienceHum_ADDITION, dataPoints->valueEnvironmentalScienceHum); 
    devStatusPtr->valueEnvironmentalScienceTemp = gizY2X(EnvironmentalScienceTemp_RATIO,  EnvironmentalScienceTemp_ADDITION, dataPoints->valueEnvironmentalScienceTemp); 
    devStatusPtr->valuesignal = gizY2X(signal_RATIO,  signal_ADDITION, dataPoints->valuesignal); 
